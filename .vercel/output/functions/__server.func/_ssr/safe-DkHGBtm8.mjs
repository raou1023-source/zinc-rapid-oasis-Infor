//#region node_modules/.nitro/vite/services/ssr/assets/safe-DkHGBtm8.js
var LOCALES = [
	"ja",
	"en",
	"zh",
...
function decodeXml(value) {
	return value.replace(/<!\[CDATA\[([\s\S]*?)\]\]>/g, "$1").replace(/&(#x[0-9a-f]+|#\d+|[a-z]+);/gi, (_, ent) => {
		if (ent[0] === "#") {
...
function cleanSummary(html) {
	...
	text = text.replace(/<script[\s\S]*?<\/script>/gi, "").replace(/<style[\s\S]*?<\/style>/gi, "").replace(/<[^>]+>/g, " ").replace(/https?:\/\/\S+/g, "").replace(/\s+/g, " ").trim();
	if (text.length < 18) return "";
	return text.slice(0, 280);
}
function safeText(value, max = 400) {
	return stripTags(value).replace(/[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/g, "").slice(0, max).trim();
}
...
function isArticleId(value) {
	return /^[a-z0-9]{1,16}$/i.test(value);
}
//#endregion
export { isArticleId as a, safeText as c, cleanSummary as i, stripTags as l, LOCALE_META as n, parseLocale as o, MESSAGES as r, safeHttpUrl as s, LOCALES as t };
